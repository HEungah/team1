...
onView() 
// 반복문 돌릴때 break; 를사용 할때는 반복문 결과 값이 한개 출력일때 사용한다
function onView(){
	// 식별자 : 인덱스 , 게시물번호=인덱스찾기
...
// 3. 삭제하기 [실행조건 : 삭제하기 버튼을 클릭했을때]
function onDelete( ){ // 인수?? 삭제할식별자x 전역변수에 있기때문에
 
 	//1. 삭제할 게시물 찾기
 	for(let i = 0 ; i<boardList.length ; i++){
		 // 2. 클릭된 게시물(현재 보고 있는게시물) 와 i번째 게시물과 번호가 같으면 
		 if(no = boardList[i].no ){
			 //3.삭제하기 
			 boardList.splice( i , 1 ); // i번째 인덱스 1개 삭제하기..
			 	// 배열에 변화된 결과를 쿠키[js메모리가 아니다]에 반영
